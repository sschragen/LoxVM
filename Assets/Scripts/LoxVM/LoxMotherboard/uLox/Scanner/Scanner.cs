...
namespace ULox
{
    ...
    public sealed class Scanner
    {
        ...
        private string GetString()
        {
            ...
            while (!IsAtEnd())
            {
                if (CurrentChar == '"'
                    && prevChar != '\\')
                {
                    return _workingSpaceStringBuilder.ToString();
                }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void EmitUsing(TokenType simpleToken, string literal)
        {
            _usings.Add(new Token(
                simpleToken,
                literal,
                _stringIterator.CurrentIndex));
        }
