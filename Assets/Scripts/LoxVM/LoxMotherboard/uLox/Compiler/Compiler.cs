    ...
    public sealed class Compiler : ICompilerDesugarContext
    {
        ...
        private void Setup()
        {
            ...
            AddStatementCompilette(
                ...
                (TokenType.SLEEP, SleepStatement),
                ...
                (TokenType.USING,UsingStatement)
                );
        ...
        public void ThrowCompilerException(string msg)
        {
            ...
            {
                throw new CompilerException(str,loc);
            }
        }
        private string LocationFromContext()
        {
            var location = $"'{CurrentChunk.GetLocationString()}'";
            ...
            return $"at position {line}:{character} in {location}";
        }
        private string MessageFromContext(string msg)
        {
            var previousToken = TokenIterator.PreviousToken;
            msg = msg + $"{LiteralStringPartial(previousToken.Literal)}.";
            return msg;
        }
        ...
        public void Declaration()
        {
            ...
            {
                //UnityEngine.Debug.LogWarning($"Token : {TokenIterator.CurrentToken.TokenType.ToString()} complette : {complette.ToString()}");
                TokenIterator.Advance();
        ...
        public static void UsingStatement(Compiler compiler)
        {
            do
            {
                compiler.TokenIterator.Consume(TokenType.STRING, $"'<LibraryName>' expected. Got {compiler.TokenIterator.CurrentToken.Literal}");
                compiler._compilingScript.Dependancies.Add(compiler.TokenIterator.PreviousToken.Literal);
                if ( !compiler.TokenIterator.Check(TokenType.END_STATEMENT) )
                {
                    compiler.TokenIterator.Consume(TokenType.COMMA, ", or ; expected.");
                }
            } while (!compiler.TokenIterator.Match(TokenType.END_STATEMENT) );
        ...
        public static void SleepStatement(Compiler compiler)
        {
            compiler.TokenIterator.Consume(TokenType.OPEN_PAREN, "Expect '(' after sleep.");
            compiler.Expression();
            compiler.TokenIterator.Consume(TokenType.CLOSE_PAREN, "Expect ')' after sleep.");

            compiler.EmitPacket(new ByteCodePacket(OpCode.SLEEP));
            compiler.ConsumeEndStatement();
        }
